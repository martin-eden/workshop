return
  function(self, node, deep, parent_type)
    ...
    elseif (node_type == 'string') then
      ...
      string_adder:add_term(quoted_node)
    elseif (node_type == 'function') then
      local node_name = self.name_giver:give_name(node)
      string_adder:add_term(quote_string(node_name))
    ...
    elseif (node_type == 'table') then
      if self.name_giver.names[node] then
        string_adder:add_term(self.name_giver.names[node])
      else
        self.name_giver:give_name(node) --to generate and store it inside
        string_adder:add_term(token:open_table(deep))
        ...
        if not is_empty then
          string_adder:add_term(token:records_delimiter(deep + 1, true))
        end
        string_adder:add_term(token:close_table(deep, is_empty))
      end
    end
  end
