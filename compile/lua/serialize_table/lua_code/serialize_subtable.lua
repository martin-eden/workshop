return
  function(self, node)
    ...
    string_adder:add_term('local ' .. node_rec.name .. ' = ')
    string_adder:add_term(token:open_table(1))
    ...
    if not is_empty then
      string_adder:add_term(token:records_delimiter(2, true))
    end
    string_adder:add_term(token:close_table(1, is_empty) .. token.commands_delimiter)
      ...
      for parent, parent_keys in pairs(node_rec.refs) do
        if self.node_recs[parent] and self.node_recs[parent].is_defined then
          ...
          for parent_key in pairs(parent_keys) do
            string_adder:add_term(self.node_recs[parent].name)
            self:put_qualified_key(parent_key)
            string_adder:add_term(' = ' .. node_rec.name .. token.commands_delimiter)
          end
        end
      end
    end
