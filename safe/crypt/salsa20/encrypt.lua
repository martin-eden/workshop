...
local handle_num_rounds = request('handle_num_rounds')

local create_block = request('!.bare.crypt.salsa20.create_block')
local shuffle_block = request('!.bare.crypt.salsa20.shuffle_block')
...
return
  function(a_plain, a_key, a_salt, a_first_block_num, a_num_rounds)
    ...
    local num_rounds = handle_num_rounds(a_num_rounds)
    ...
    for i = 1, num_blocks do
      local result_block = shuffle_block(base_block, shifts, num_rounds)
